...
namespace Tetris {
    public class l_Block : Block {
        ...
        public l_Block() {
            ...
            point2 = new Point(0, 5, color);
            point3 = new Point(0, 6, color);
            blockType = 1;
            state = 0;
        }
        ...
        public override bool canRotate(Graph graph) {
            /* 
             * 注意！！！
             * 此处必须以core为基准，因为point1、point2、point3会改变
             */

            //下面2五个点相对于core的位置是不会因state而改变
            Point p1 = new Point(core.X - 1, core.Y - 1, color);  //[0,0]
            Point p2 = new Point(core.X + 1, core.Y + 1, color);  //[2,2]
            Point p3 = new Point(core.X + 1, core.Y + 2, color);  //[2,3]
            Point p4 = new Point(core.X + 2, core.Y + 1, color);  //[3,2]
            Point p5 = new Point(core.X + 2, core.Y + 2, color);  //[3,3]

            //这三个点相对于core会因state而改变
            Point p6 = new Point();
            Point p7 = new Point();
            Point p8 = new Point();
            ...
            if (state == 0) {  //状态0
                //设此时有一个2维数组[4,4]
                p6 = new Point(core.X - 1, core.Y, color);  //[0,1]
                p7 = new Point(core.X + 1, core.Y, color);  //[2,1]
                p8 = new Point(core.X + 2, core.Y, color);  //[3,1]
            }
            else if (state == 1) {  //状态1
                p6 = new Point(core.X, core.Y - 1, color);  //[1,0]
                p7 = new Point(core.X, core.Y + 1, color);  //[1,2]
                p8 = new Point(core.X, core.Y + 2, color);  //[1,3]
            }
            else {
                throw new Exception("状态错误");
            }
            if (!p1.Valid || !p2.Valid || !p3.Valid || !p4.Valid
                || !p5.Valid || !p6.Valid || !p7.Valid || !p8.Valid)
                return false;  //越界
            //图上对应任意一点有值，即有障碍，不能翻转
            if (graph.getValue(p1) != 0 || graph.getValue(p2) != 0
                || graph.getValue(p3) != 0 || graph.getValue(p4) != 0
                || graph.getValue(p5) != 0 || graph.getValue(p6) != 0
                || graph.getValue(p7) != 0 || graph.getValue(p8) != 0)
                return false;  //翻转失败，不进行任何操作
        ...
        public override bool canMoveLeft(Graph graph) {
            if (state == 0) {
                ...
                if (!p1.Valid) return false;  //越界
                if (graph.getValue(p1) != 0) return false;  //有障碍
            }
            else if (state == 1) {
                Point p1 = new Point(point1.X, point1.Y - 1, color);
                Point p2 = new Point(core.X, core.Y - 1, color);
                Point p3 = new Point(point2.X, point2.Y - 1, color);
                Point p4 = new Point(point3.X, point3.Y - 1, color);
                if (!p1.Valid || !p2.Valid || !p3.Valid || !p4.Valid) return false;  //越界
                if (graph.getValue(p1) != 0 || graph.getValue(p2) != 0
                    || graph.getValue(p3) != 0 || graph.getValue(p4) != 0)
                    return false;  //有障碍
            }
        ...
        public override bool canMoveDown(Graph graph) {
            if (state == 0) {
                ...
                Point p3 = new Point(point2.X + 1, point2.Y, color);
                Point p4 = new Point(point3.X + 1, point3.Y, color);
                if (!p1.Valid || !p2.Valid || !p3.Valid || !p4.Valid) return false;  //越界
                if (graph.getValue(p1) != 0 || graph.getValue(p2) != 0
                    || graph.getValue(p3) != 0 || graph.getValue(p4) != 0)
                    return false;  //有障碍
