...
namespace Tetris {
    public class T_Block : Block {
        ...
        public T_Block() {
            ...
            point1 = new Point(1, 3, color);
            point2 = new Point(0, 4, color);
        ...
        public override bool canRotate(Graph graph) {
            //这四个点相对于core的位置不因state为改变
            Point p1 = new Point(core.X - 1, core.Y - 1, color);  //[0,0]
            Point p2 = new Point(core.X - 1, core.Y + 1, color);  //[0,2]
            Point p3 = new Point(core.X + 1, core.Y - 1, color);  //[2,0]
            Point p4 = new Point(core.X + 1, core.Y + 1, color);  //[2,2]

            //下面的一个点相对于core的位置会因state而改变
            Point p5;

            if (state == 0) {
                p5 = new Point(core.X + 1, core.Y, color);  //[2,1]

                //状态0逆时针旋转时，和p4无关
                if (!p1.Valid || !p2.Valid || !p3.Valid || !p5.Valid) return false;
                if (graph.getValue(p1) != 0 || graph.getValue(p2) != 0
                    || graph.getValue(p3) != 0 || graph.getValue(p5) != 0)
                    return false;
            }
            else if (state == 1) {
                p5 = new Point(core.X, core.Y + 1, color);  //[1,2]

                //状态1逆时针旋转时，和p2无关
                if (!p1.Valid || !p3.Valid || !p4.Valid || !p5.Valid) return false;
                if (graph.getValue(p1) != 0 || graph.getValue(p3) != 0
                    || graph.getValue(p4) != 0 || graph.getValue(p5) != 0)
                    return false;
            }
            else if (state == 2) {
                p5 = new Point(core.X - 1, core.Y, color);  //[0,1]

                //状态2逆时针旋转时，和p1无关
                if (!p2.Valid || !p3.Valid || !p4.Valid || !p5.Valid) return false;
                if (graph.getValue(p2) != 0 || graph.getValue(p3) != 0
                    || graph.getValue(p4) != 0 || graph.getValue(p5) != 0)
                    return false;
            }
            else if (state == 3) {
                p5 = new Point(core.X, core.Y - 1, color);  //[1,0]

                //状态3逆时针旋转时，和p3无关
                if (!p1.Valid || !p2.Valid || !p4.Valid || !p5.Valid) return false;
                if (graph.getValue(p1) != 0 || graph.getValue(p2) != 0
                    || graph.getValue(p4) != 0 || graph.getValue(p5) != 0)
                    return false;
            }
        ...
        public override bool canMoveLeft(Graph graph) {
            if (state == 0) {
                ...
                if (!p1.Valid || !p2.Valid || !p3.Valid) return false;  //越界
        ...
        public override bool canMoveRight(Graph graph) {
            if (state == 0) {
                ...
                if (!p1.Valid || !p2.Valid || !p3.Valid) return false;  //越界
                if (graph.getValue(p1) != 0 || graph.getValue(p2) != 0 || graph.getValue(p3) != 0) return false;  //有障碍
            }
            else if (state == 2) {
                Point p1 = new Point(point1.X, point1.Y + 1, color);
                Point p2 = new Point(point2.X, point2.Y + 1, color);
